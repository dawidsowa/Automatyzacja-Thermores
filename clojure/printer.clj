...
(def doktoranci (map clojure.walk/keywordize-keys (json/parse-stream (clojure.java.io/reader "./doktoranci.json"))))
(defn mailto [email] [:a
...
(defn box [& {:keys [name id tel keywords promotor text temat_pracy_doktorskiej photo]}]
  (let [pairs [["Temat pracy doktorskiej" temat_pracy_doktorskiej]
               ["Promotor" promotor]
               ["Obszar badań" text]
               ["Słowa kluczowe" (join ", " keywords)]
               ["Biuro" "bud. D-2 pok. 13"]
               ["Telefon" (telto tel)]
